/*
	Using the JavaScript language, have the function ArithGeo(arr) take the array of numbers stored in arr and return the string "Arithmetic"
...
function ArithGeo(arr) { 
  var ratArith = arr[1] - arr[0];
  var ratGeo = arr[1] / arr[0];
  var retunValue = "";
  for (var i = 2; i < arr.length; i++) {
    if (arr[i] - arr[i-1] == ratArith) {
     	retunValue = "Arithmetic" 
    } else {
        retunValue = "";
        break;
    }  
  ...
  if (retunValue == "") {
	for (var i = 2; i < arr.length; i++) {
		if (arr[i] / arr[i-1] == ratGeo) {
			retunValue = "Geometric" 
		} else {
			retunValue = "";
		break;
		}  
  ...
  if ( retunValue == "") { retunValue = "-1"}
  return retunValue; 
         
}

//Test
console.log(ArithGeo([-5,5,10,15]));
